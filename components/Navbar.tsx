...
const Navbar: React.FC<NavbarProps> = ({ onOpenBooking }) => {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 w-full z-[60] transition-all duration-500 ${isScrolled || isMobileMenuOpen ? 'bg-black/95 py-3 border-b border-gold/20' : 'bg-transparent py-8'}`}>
        <div className="container mx-auto px-6 flex justify-between items-center">
          {/* Logo */}
          <a href="#inicio" onClick={handleLinkClick} className="flex flex-col items-center group">
          ...
          <div className="hidden lg:flex space-x-10 text-[11px] uppercase tracking-[0.2em] font-bold items-center">
            {navLinks.map((link) => (
              <a key={link.name} href={link.href} className="hover-gold transition">
                {link.name}
              </a>
            ))}
            <button 
          ...
          >
            <div className="w-6 h-5 relative flex flex-col justify-between">
              <span className={`w-full h-px bg-current transition-transform duration-300 ${isMobileMenuOpen ? 'rotate-45 translate-y-2' : ''}`}></span>
              <span className={`w-full h-px bg-current transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-0' : ''}`}></span>
              <span className={`w-full h-px bg-current transition-transform duration-300 ${isMobileMenuOpen ? '-rotate-45 -translate-y-2' : ''}`}></span>
            </div>
          </button>
        </div>
      ...
      {/* Mobile Menu Overlay */}
      <div className={`fixed inset-0 z-[55] bg-black transition-transform duration-500 lg:hidden ${isMobileMenuOpen ? 'translate-y-0' : '-translate-y-full'}`}>
